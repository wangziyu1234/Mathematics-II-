...
\vfill
\newpage
{\large\bfseries 2020.9}

设 $\begin{cases} x = \sqrt{t^2+1}, \\ y = \ln(t + \sqrt{t^2+1}), \end{cases}$ 则 $\left.\dfrac{\mathrm{d}^2 y}{\mathrm{d}x^2}\right|_{t=1} =$ \underline{\hspace{20mm}}．

\vfill
\newpage
{\large\bfseries 2020.10}

$\displaystyle\int_{0}^{1}\mathrm{d}y\int_{\sqrt{y}}^{1} \sqrt{x^3+1}\,\mathrm{d}x =$ \underline{\hspace{20mm}}．

\vfill
\newpage
{\large\bfseries 2020.11}

设 $z = \arctan[xy + \sin(x+y)]$，则 $\mathrm{d}z\big|_{(0,\pi)} =$ \underline{\hspace{20mm}}．

\vfill
\newpage
{\large\bfseries 2020.12}

斜边长为 $2a$ 的等腰直角三角形平板铅直地沉入水中，且斜边与水面相齐，记重力加速度为 $g$，水密度为 $\rho$，则该平板一侧所受的水压力为 \underline{\hspace{20mm}}．

\vfill
\newpage
{\large\bfseries 2020.13}

设 $y = y(x)$ 满足 $y'' + 2y' + y = 0$，且 $y(0) = 0$，$y'(0) = 1$，则 $\displaystyle\int_{0}^{+\infty} y(x)\,\mathrm{d}x =$ \underline{\hspace{20mm}}．

\vfill
\newpage
{\large\bfseries 2020.14}

行列式 $\begin{vmatrix} a & 0 & -1 & 1 \\ 0 & a & 1 & -1 \\ -1 & 1 & a & 0 \\ 1 & -1 & 0 & a \end{vmatrix} =$ \underline{\hspace{20mm}}．

\vfill
\newpage
{\large\bfseries 2020.15}
