...
\\
\noindent （A）比 $x$ 高阶的无穷小量
\\
\noindent （B）比 $x$ 低阶的无穷小量
\\
\noindent （C）与 $x$ 同阶但不等价的无穷小量
\\
\noindent （D）与 $x$ 等价的无穷小量
...
\\
\noindent （A）$x = \pi$ 是函数 $F(x)$ 的跳跃间断点
\\
\noindent （B）$x = \pi$ 是函数 $F(x)$ 的可去间断点
\\
\noindent （C）$F(x)$ 在 $x = \pi$ 处连续但不可导
\\
\noindent （D）$F(x)$ 在 $x = \pi$ 处可导
...
\noindent （B）$-2yf'(xy)$
\\
\noindent （C）$\frac{2}{x}f(xy)$
\\
\noindent （D）$-\frac{2}{x}f(xy)$
...
\\
\noindent （A）矩阵 $\boldsymbol{C}$ 的行向量组与矩阵 $\boldsymbol{A}$ 的行向量组等价
\\
\noindent （B）矩阵 $\boldsymbol{C}$ 的列向量组与矩阵 $\boldsymbol{A}$ 的列向量组等价
\\
\noindent （C）矩阵 $\boldsymbol{C}$ 的行向量组与矩阵 $\boldsymbol{B}$ 的行向量组等价
\\
\noindent （D）矩阵 $\boldsymbol{C}$ 的列向量组与矩阵 $\boldsymbol{B}$ 的列向量组等价
...
\noindent （A）$a = 0, b = 2$
\\
\noindent （B）$a = 0, b$ 为任意常数
\\
\noindent （C）$a = 2, b = 0$
\\
\noindent （D）$a = 2, b$ 为任意常数

\vfill
\newpage
\qn{2013.9}
$\lim_{x \to 0}\left[2 - \frac{\ln(1 + x)}{x}\right]^{\frac{1}{x}} =$ ______.

\vfill
\newpage
\qn{2013.10}
设函数 $f(x) = \int_{-1}^x \sqrt{1 - e^t}dt$，则 $y = f(x)$ 的反函数 $x = f^{-1}(y)$ 在 $y = 0$ 处的导数 $\left.\frac{dx}{dy}\right|_{y = 0} =$ ______.

\vfill
\newpage
\qn{2013.11}
设封闭曲线 $L$ 的极坐标方程为 $r = \cos 3\theta$（$-\frac{\pi}{6} \le \theta \le \frac{\pi}{6}$），则 $L$ 所围平面图形的面积是 ______.

\vfill
\newpage
\qn{2013.12}
曲线 $\begin{cases} x = \arctan t, \\ y = \ln\sqrt{1 + t^2}, \end{cases}$ 上对应于 $t = 1$ 的点处的法线方程为 ______.

\vfill
\newpage
\qn{2013.13}
已知 $y_1 = e^{3x} - xe^{2x}, y_2 = e^x - xe^{2x}, y_3 = -xe^{2x}$ 是某二阶常系数非齐次线性微分方程的3个解，则该方程满足条件 $y|_{x = 0} = 0, y'|_{x = 0} = 1$ 的解为 $y =$ ______.

\vfill
\newpage
\qn{2013.14}
设 $\boldsymbol{A} = (a_{ij})$ 是3阶非零矩阵，$|\boldsymbol{A}|$ 为 $\boldsymbol{A}$ 的行列式，$A_{ij}$ 为 $a_{ij}$ 的代数余子式。若 $a_{ij} + A_{ij} = 0$（$i, j = 1, 2, 3$），则 $|\boldsymbol{A}| =$ ______.
