...
\newpage
\qn{2008.2}
如图，曲线段的方程为 $y = f(x)$，函数 $f(x)$ 在区间 $[0, a]$ 上有连续导数，则定积分 $\int_0^a x f'(x) \,\mathrm{d}x$ 等于（　）
\\
\noindent （A）曲边梯形 $ABOD$ 的面积
\\
\noindent （B）梯形 $ABOD$ 的面积
\\
\noindent （C）曲边三角形 $ACD$ 的面积
\\
\noindent （D）三角形 $ACD$ 的面积
...
\\
\noindent （A）$y''' + y'' - 4y' - 4y = 0$
\\
\noindent （B）$y''' + y'' + 4y' + 4y = 0$
\\
\noindent （C）$y''' - y'' - 4y' + 4y = 0$
\\
\noindent （D）$y''' - y'' + 4y' - 4y = 0$
...
\\
\noindent （A）1个可去间断点，1个跳跃间断点
\\
\noindent （B）1个可去间断点，1个无穷间断点
\\
\noindent （C）2个跳跃间断点
\\
\noindent （D）2个无穷间断点
...
\\
\noindent （A）若 $\{x_n\}$ 收敛，则 $\{f(x_n)\}$ 收敛
\\
\noindent （B）若 $\{x_n\}$ 单调，则 $\{f(x_n)\}$ 收敛
\\
\noindent （C）若 $\{f(x_n)\}$ 收敛，则 $\{x_n\}$ 收敛
\\
\noindent （D）若 $\{f(x_n)\}$ 单调，则 $\{x_n\}$ 收敛

\vfill
\newpage
\qn{2008.6}
设函数 $f$ 连续。若 $F(u, v) = \iint_{D_{uv}} \dfrac{f(x^2 + y^2)}{\sqrt{x^2 + y^2}} \,\mathrm{d}x\,\mathrm{d}y$，其中区域 $D_{uv}$ 为图中阴影部分，则 $\dfrac{\partial F}{\partial u} =$（　）
\\
\noindent （A）$v f(u^2)$
\\
\noindent （B）$\dfrac{v}{u} f(u^2)$
\\
\noindent （C）$v f(u)$
\\
\noindent （D）$\dfrac{v}{u} f(u)$
...
\\
\noindent （A）$\boldsymbol{E} - \boldsymbol{A}$ 不可逆，$\boldsymbol{E} + \boldsymbol{A}$ 不可逆
\\
\noindent （B）$\boldsymbol{E} - \boldsymbol{A}$ 不可逆，$\boldsymbol{E} + \boldsymbol{A}$ 可逆
\\
\noindent （C）$\boldsymbol{E} - \boldsymbol{A}$ 可逆，$\boldsymbol{E} + \boldsymbol{A}$ 可逆
\\
\noindent （D）$\boldsymbol{E} - \boldsymbol{A}$ 可逆，$\boldsymbol{E} + \boldsymbol{A}$ 不可逆

\vfill
\newpage
\qn{2008.8}
设 $\boldsymbol{A} = \begin{pmatrix} 1 & 2 \\ 2 & 1 \end{pmatrix}$，则在实数域上与 $\boldsymbol{A}$ 合同的矩阵为（　）
\\
\noindent （A）$\begin{pmatrix} -2 & 1 \\ 1 & -2 \end{pmatrix}$
\\
\noindent （B）$\begin{pmatrix} 2 & -1 \\ -1 & 2 \end{pmatrix}$
\\
\noindent （C）$\begin{pmatrix} 2 & 1 \\ 1 & 2 \end{pmatrix}$
\\
\noindent （D）$\begin{pmatrix} 1 & -2 \\ -2 & 1 \end{pmatrix}$

\vfill
\newpage
\qn{2008.9}
已知函数 $f(x)$ 连续，且 $\lim_{x \to 0} \dfrac{1 - \cos[x f(x)]}{(e^{x^2} - 1) f(x)} = 1$，则 $f(0) =$ ________.

\vfill
\newpage
\qn{2008.10}
微分方程 $(y + x^2 e^{-x}) \,\mathrm{d}x - x \,\mathrm{d}y = 0$ 的通解是 $y =$ ________.

\vfill
\newpage
\qn{2008.11}
曲线 $\sin(xy) + \ln(y - x) = x$ 在点 $(0, 1)$ 处的切线方程是 ________.
...
\newpage
\qn{2008.13}
设 $z = \left(\dfrac{y}{x}\right)^{\frac{x}{y}}$，则 $\left. \dfrac{\partial z}{\partial x} \right|_{(1,2)} =$ ________.

\vfill
\newpage
\qn{2008.14}
设3阶矩阵 $\boldsymbol{A}$ 的特征值为 $2, 3, \lambda$。若行列式 $|2\boldsymbol{A}| = -48$，则 $\lambda =$ ________.
