...
\vfill
\newpage
{\large\bfseries 2010.9}

3阶常系数线性齐次微分方程 $y''' - 2y'' + y' - 2y = 0$ 的通解为 $y =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2010.10}

曲线 $y = \dfrac{2x^3}{x^2 + 1}$ 的渐近线方程为 \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2010.11}

函数 $y = \ln(1 - 2x)$ 在 $x = 0$ 处的 $n$ 阶导数 $y^{(n)}(0) =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2010.12}

当 $0 \leqslant \theta \leqslant \pi$ 时，对数螺线 $r = e^{\theta}$ 的弧长为 \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2010.13}

已知一个长方形的长 $l$ 以 $2\,\text{cm/s}$ 的速率增加，宽 $w$ 以 $3\,\text{cm/s}$ 的速率增加，则当 $l = 12\,\text{cm}, \; w = 5\,\text{cm}$ 时，它的对角线增加的速率为 \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2010.14}

设 $\boldsymbol{A}, \boldsymbol{B}$ 为3阶矩阵，且 $|\boldsymbol{A}| = 3, \; |\boldsymbol{B}| = 2, \; |\boldsymbol{A}^{-1} + \boldsymbol{B}| = 2$，则 $|\boldsymbol{A} + \boldsymbol{B}^{-1}| =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2010.15}
...
\begin{center}\includegraphics[width=.62\textwidth,height=65mm,keepaspectratio]{_题图/2010.18.png}\end{center}

\vfill
\newpage
{\large\bfseries 2010.19}
