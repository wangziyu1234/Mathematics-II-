...
\vfill
\newpage
{\large\bfseries 2015.4}

设函数 $f(x)$ 在 $(-\infty, +\infty)$ 内连续，其2阶导函数 $f''(x)$ 的图形如右图所示，则曲线 $y = f(x)$ 的拐点个数为（　）

\begin{center}\includegraphics[width=.62\textwidth,height=65mm,keepaspectratio]{_题图/2015.4.png}\end{center}
...
\vfill
\newpage
{\large\bfseries 2015.9}

设 $\begin{cases} x = \arctan t, \\ y = 3t + t^3, \end{cases}$ 则 $\left.\frac{\mathrm{d}^2y}{\mathrm{d}x^2}\right|_{t = 1} =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2015.10}

函数 $f(x) = x^2 2^x$ 在 $x = 0$ 处的 $n$ 阶导数 $f^{(n)}(0) =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2015.11}

设函数 $f(x)$ 连续，$\varphi(x) = \int_0^{x^2} xf(t)\,\mathrm{d}t$. 若 $\varphi(1) = 1$，$\varphi'(1) = 5$，则 $f(1) =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2015.12}

设函数 $y = y(x)$ 是微分方程 $y'' + y' - 2y = 0$ 的解，且在 $x = 0$ 处 $y(x)$ 取得极值3，则 $y(x) =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2015.13}

若函数 $z = z(x, y)$ 由方程 $e^{x + 2y + 3z} + xyz = 1$ 确定，则 $\mathrm{d}z|_{(0, 0)} =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2015.14}

设3阶矩阵 $\boldsymbol{A}$ 的特征值为 $2, -2, 1$，$\boldsymbol{B} = \boldsymbol{A}^2 - \boldsymbol{A} + \boldsymbol{E}$，其中 $\boldsymbol{E}$ 为3阶单位矩阵，则行列式 $|\boldsymbol{B}| =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2015.15}
