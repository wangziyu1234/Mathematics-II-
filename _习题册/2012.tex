...
\\
\noindent （A）$I_1 < I_2 < I_3$
\\
\noindent （B）$I_3 < I_2 < I_1$
\\
\noindent （C）$I_2 < I_3 < I_1$
\\
\noindent （D）$I_2 < I_1 < I_3$
...
\\
\noindent （A）$x_1 > x_2, y_1 < y_2$
\\
\noindent （B）$x_1 > x_2, y_1 > y_2$
\\
\noindent （C）$x_1 < x_2, y_1 < y_2$
\\
\noindent （D）$x_1 < x_2, y_1 > y_2$
...
\\
\noindent （A）$\boldsymbol{\alpha}_1, \boldsymbol{\alpha}_2, \boldsymbol{\alpha}_3$
\\
\noindent （B）$\boldsymbol{\alpha}_1, \boldsymbol{\alpha}_2, \boldsymbol{\alpha}_4$
\\
\noindent （C）$\boldsymbol{\alpha}_1, \boldsymbol{\alpha}_3, \boldsymbol{\alpha}_4$
\\
...
\\
\noindent （A）$\begin{pmatrix} 1 & 0 & 0 \\ 0 & 2 & 0 \\ 0 & 0 & 1 \end{pmatrix}$
\\
\noindent （B）$\begin{pmatrix} 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 2 \end{pmatrix}$
\\
\noindent （C）$\begin{pmatrix} 2 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 2 \end{pmatrix}$
\\
\noindent （D）$\begin{pmatrix} 2 & 0 & 0 \\ 0 & 2 & 0 \\ 0 & 0 & 1 \end{pmatrix}$

\vfill
\newpage
\qn{2012.9}
设 $y = y(x)$ 是由方程 $x^2 - y + 1 = e^y$ 所确定的隐函数，则 $\left.\frac{d^2y}{dx^2}\right|_{x = 0} =$ ______.

\vfill
\newpage
\qn{2012.10}
$\lim_{n \to \infty}n\left(\frac{1}{1 + n^2} + \frac{1}{2^2 + n^2} + \cdots + \frac{1}{n^2 + n^2}\right) =$ ______.

\vfill
\newpage
\qn{2012.11}
设 $z = f\left(\ln x + \frac{1}{y}\right)$，其中函数 $f(u)$ 可微，则 $x\frac{\partial z}{\partial x} + y^2\frac{\partial z}{\partial y} =$ ______.

\vfill
\newpage
\qn{2012.12}
微分方程 $y dx + (x - 3y^2)dy = 0$ 满足条件 $y|_{x = 1} = 1$ 的解为 $y =$ ______.

\vfill
\newpage
\qn{2012.13}
曲线 $y = x^2 + x$（$x < 0$）上曲率为 $\frac{\sqrt{2}}{2}$ 的点的坐标是 ______.

\vfill
\newpage
\qn{2012.14}
设 $\boldsymbol{A}$ 为3阶矩阵，$|\boldsymbol{A}| = 3$，$\boldsymbol{A}^*$ 为 $\boldsymbol{A}$ 的伴随矩阵，若交换 $\boldsymbol{A}$ 的第1行与第2行得矩阵 $\boldsymbol{B}$，则 $|\boldsymbol{BA}^*| =$ ______.
