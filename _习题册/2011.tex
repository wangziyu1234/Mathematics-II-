...
\vfill
\newpage
{\large\bfseries 2011.9}

$\lim_{x \to 0}\left(\frac{1 + 2^x}{2}\right)^{\frac{1}{x}} =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2011.10}

微分方程 $y' + y = e^{-x}\cos x$ 满足条件 $y(0) = 0$ 的解为 $y =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2011.11}

曲线 $y = \int_0^x \tan t dt$（$0 \le x \le \frac{\pi}{4}$）的弧长 $s =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2011.12}

设函数 $f(x) = \begin{cases} \lambda e^{-\lambda x}, & x > 0, \\ 0, & x \le 0, \end{cases} \lambda > 0$，则 $\int_{-\infty}^{+\infty} x f(x)dx =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2011.13}

设平面区域 $D$ 由直线 $y = x$，圆 $x^2 + y^2 = 2y$ 及 $y$ 轴所围成，则二重积分 $\iint_D xy d\sigma =$ \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2011.14}

二次型 $f(x_1, x_2, x_3) = x_1^2 + 3x_2^2 + x_3^2 + 2x_1x_2 + 2x_1x_3 + 2x_2x_3$，则 $f$ 的正惯性指数为 \underline{\hspace{20mm}}.

\vfill
\newpage
{\large\bfseries 2011.15}
...
\vfill
\newpage
{\large\bfseries 2011.20}

（本题满分11分）

一容器的内侧是由图中曲线绕 $y$ 轴旋转一周而成的曲面，该曲线由 $x^2 + y^2 = 2y$（$y \ge \frac{1}{2}$）与 $x^2 + y^2 = 1$（$y \le \frac{1}{2}$）连接而成。

\begin{center}\includegraphics[width=.62\textwidth,height=65mm,keepaspectratio]{_题图/2011.20.png}\end{center}
