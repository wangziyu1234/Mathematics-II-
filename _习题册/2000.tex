...
\\
\noindent (A) $a < 0, b < 0$.　　　　　(B) $a > 0, b > 0$.
\\
\noindent (C) $a \leqslant 0, b > 0$.　　　　　(D) $a \geqslant 0, b < 0$.
...
\\
\noindent (A) $f(0)$ 是 $f(x)$ 的极大值.
\\
\noindent (B) $f(0)$ 是 $f(x)$ 的极小值.
\\
\noindent (C) 点 $(0, f(0))$ 是曲线 $y = f(x)$ 的拐点.
\\
\noindent (D) $f(0)$ 不是 $f(x)$ 的极值，点 $(0, f(0))$ 也不是曲线 $y = f(x)$ 的拐点.
...
\\
\noindent (A) $f(x)g(b) > f(b)g(x)$.　　　　　(B) $f(x)g(a) > f(a)g(x)$.
\\
\noindent (C) $f(x)g(x) > f(b)g(b)$.　　　　　(D) $f(x)g(x) > f(a)g(a)$.
...
\noindent (B) $y''' + y'' - y' - y = 0$.
\\
\noindent (C) $y''' - 6y'' + 11y' - 6y = 0$.
\\
\noindent (D) $y''' - 2y'' - y' + 2y = 0$.
...
\newpage
\qn{2000.11}
当 $n$ 为正整数，且 $n\pi \leqslant x < (n + 1)\pi$ 时，证明：$2n \leqslant S(x) < 2(n + 1)$；

\vfill
\newpage
\qn{2000.12}
求 $\lim_{x \to +\infty} \dfrac{S(x)}{x}$.
...
\newpage
\qn{2000.14}
证明：当 $x \geqslant 0$ 时，不等式 $e^{-x} \leqslant f(x) \leqslant 1$ 成立.
